...
const utilFuns = new utils();

if (!fs.existsSync(BASE_DIR_PATH)) {
  fs.mkdirSync(BASE_DIR_PATH);
}

const getCronContent = utilFuns.getCronContent;
const getBootScriptPath = utilFuns.getBootScriptPath;
const formatError = utilFuns.formatError;
const getProjectTag = utilFuns.getProjectTag;
const getEnvVariables = utilFuns.getEnvVariables;
const isValidCronErr = utilFuns.isValidCronErr;
const getScriptIndex = utilFuns.getScriptIndex;
  ...
  constructor() {
    this._status = function (projectName) {
      ...
      if (isLinux) {
        ...
        } else if (cronContent.indexOf(getProjectTag(projectName)) > -1) {
          return { status: true };
        }
    ...
    this._enable = function (projectName, projectPath, envVariables) {
      ...
      if (isLinux) {
        ...
        let scriptContent = getCronContent(projectPath);
        if (cronContent.indexOf(scriptContent) < 0) {
    ...
    this._disable = function (projectName) {
      ...
      if (isLinux) {
        ...
        if (cronContent && cronContent.length > 0) {
          ...
          let scriptIndex = getScriptIndex(projectName, cronContent);
          if (scriptIndex > -1) {
    ...
    this._view = function (projectName) {
      ...
      if (isLinux) {
        ...
        if (scriptIndex > -1) {
          ...
          envVars = cronContent[scriptIndex].split(`${getProjectTag(projectName)}_`)[1].split(' ')[0];
