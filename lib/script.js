  ...
  constructor() {
    ...
    this._enable = function (projectName, projectPath, envVariables) {
      ...
      if (isLinux) {
        ...
        let scriptContent = getCronContent(projectPath, envVars, executionCmd);
      ...
      if (isLinux) {
        ...
        for (let i = 0; i < scriptContent.length; i++) {
          ...
          if (scriptContent[i].indexOf('path = ') > -1) {
            path = scriptContent[i].split('path = ')[1].trim();
          }
