  ...
  switch (true) {
    ...
    case process.argv.slice(2)[0].toString().trim() === 'view':
      ...
      if (viewResult && viewResult.err) {
        ...
        console.log(`------------------------${utilFuncs._getDottedLines(parseInt(viewName.toString().trim().length) + 2)}`);
        console.log(`Path... ${viewResult.path}`);
        if (viewResult.envVars && viewResult.envVars.length > 0) {
