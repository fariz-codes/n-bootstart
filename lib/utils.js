...
class utils {
  constructor() {
    this.getCronContent = function (projectPath, envVars, executionCmd) {
      return `\n@reboot ${envVars.split(',').join(' ')} ${executionCmd} ${path.resolve(projectPath)}\n`;
    };
    ...
    this.getProjectInfo = function (projectPath) {
      let splittedPath = path.resolve(projectPath).split('\\');
      let directory = '';
